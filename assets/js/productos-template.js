...
/**
 * SOLUCIÓN: Modificar la inicialización de filtros para evitar activación automática
 * Agregar este código al archivo assets/js/productos-template.js
 */

jQuery(document).ready(function($) {
    console.log('Productos Template Script - Versión con solución para filtros preseleccionados');
    ...
    // Variable global para rastrear si es la primera carga
     ...
     * Con lógica para evitar la recarga en el inicio
     */
    function filterProducts(page, skipInitialRequest) {
        // Asignar página actual
        page = parseInt(page) || 1;
        currentFilters.page = page;
        
        // Si debemos omitir la solicitud inicial
        if (skipInitialRequest && isInitialLoad) {
            console.log('Omitiendo solicitud AJAX inicial para prevenir recarga automática');
            isInitialLoad = false;
            return;
        }
...
// Recopilar valores de filtros actuales
        ...
        // Asegurar que el formulario de búsqueda muestre el término buscado
...
function extractSearchFromUrl() {
   ...
   /**
     * SOLUCIÓN: Modificar bindFilterEvents para evitar activación automática en el inicio
     */
    function bindFilterEvents() {
        ...
        // Filtros de categoría - Con manejo especial para primera carga
        $('.wc-productos-template .filtro-category').on('change', function(e) {
            // Verificar si el cambio fue provocado por un usuario y no programáticamente
            if (e.originalEvent || !isInitialLoad) {
                console.log('Categoría cambiada por usuario');
                filterProducts(1); // Volver a página 1 al cambiar filtro
            } else {
                console.log('Ignorando cambio automático durante carga inicial');
            }
        });
        
        // Filtros de grado - Con manejo especial para primera carga
        $('.wc-productos-template .filtro-grade').on('change', function(e) {
            // Solo procesar si es interacción real de usuario
            if (e.originalEvent || !isInitialLoad) {
                console.log('Grado cambiado por usuario');
                filterProducts(1); // Volver a página 1 al cambiar filtro
            } else {
                console.log('Ignorando cambio automático durante carga inicial');
            }
...
$(document).ready(function($) {
   ...
   /**
     * Versión mejorada del método init
     */
    function init() {
        console.log('Inicializando productos template con solución para filtros preseleccionados');
        ...
        // Extraer término de búsqueda de la URL (parámetro 's')
        if (urlParams.has('s')) {
            var searchTerm = urlParams.get('s');
            $('.wc-productos-template .productos-search input').val(searchTerm);
            currentFilters.search = searchTerm;
        ...
        // SOLUCIÓN: Marcar el estado inicial pero NO realizar la carga AJAX automática
        isInitialLoad = true;
        ...
        $(window).on('load', function() {
            ...
            // Ya no es la carga inicial después de que la ventana esté completamente cargada
            isInitialLoad = false;
        });
