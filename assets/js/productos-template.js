...
// Solución definitiva para eliminar elementos huérfanos en la cuadrícula
jQuery(document).ready(function($) {
    function limpiarElementosHuerfanos() {
        console.log("Ejecutando limpieza de elementos huérfanos...");
        
        // Calcular productos por fila según el ancho de pantalla
        var productosPorFila = 4; // Valor por defecto para pantallas grandes
        
        if (window.innerWidth <= 480) {
            productosPorFila = 2; // Móviles
        } else if (window.innerWidth <= 768) {
            productosPorFila = 2; // Tablets pequeñas
        } else if (window.innerWidth <= 991) {
            productosPorFila = 3; // Tablets y pantallas medianas
        }
        
        // Determinar el número máximo de productos a mostrar (3 filas)
        var maxProductos = productosPorFila * 3;
        
        // Seleccionar todos los contenedores de productos
        var $contenedores = $('.productos-grid, ul.products, .wc-productos-template ul.products');
        
        // Para cada contenedor de productos
        $contenedores.each(function() {
            var $grid = $(this);
            
            // 1. Eliminar cualquier div.producto-interior que esté directamente en el contenedor
            $grid.children('div.producto-interior').remove();
            
            // 2. Ocultar productos después del límite de 3 filas
            $grid.children('li.product:nth-child(n+' + (maxProductos + 1) + ')').hide();
            
            // 3. Aplicar CSS para asegurar solo 3 filas
            $grid.css({
                'display': 'grid',
                'grid-template-rows': 'repeat(3, auto)',
                'grid-auto-rows': '0px',
        ...
        // 4. Buscar en todo el documento cualquier div.producto-interior huérfano
        $('div.producto-interior').each(function() {
            var $div = $(this);
            // Si no está dentro de un li.product, eliminarlo
            if ($div.parents('li.product').length === 0) {
                $div.remove();
            }
        });
        
        // 5. Forzar que no existan elementos después de la tercera fila
        $('body').append('<style id="forzar-tres-filas">' +
            '.productos-grid > *:nth-child(n+' + (maxProductos + 1) + '), ' +
            'ul.products > *:nth-child(n+' + (maxProductos + 1) + ') {' +
    ...
    $(window).on('load', limpiarElementosHuerfanos);
    
    // Ejecutar después de cambiar el tamaño de la ventana
    $(window).on('resize', limpiarElementosHuerfanos);
    
    // Ejecutar después de completar cualquier solicitud AJAX
    $(document).ajaxComplete(limpiarElementosHuerfanos);
